...
export default class SmartHome implements Mediator {
  ...
  participantChanged(participant: Participant): void {
    if (participant === this.door && !this.door.getIsClosed) {
      this.aircon.off();
      this.boiler.off();
    }

    if (participant === this.window && !this.window.getIsClosed) {
      this.aircon.off();
      this.boiler.off();
    }
  ...
  install() {
    ...
    domDoor.addEventListener("click", () => {
      this.door.getIsClosed ? this.door.open() : this.door.close();
      this.display();
    });
    domWindow.addEventListener("click", () => {
      this.window.getIsClosed ? this.window.open() : this.window.close();
      this.display();
    });
